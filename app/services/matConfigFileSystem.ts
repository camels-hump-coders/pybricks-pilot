...
class MatConfigFileSystem {
  ...
  async discoverMats(dirHandle: FileSystemDirectoryHandle): Promise<MatMetadata[]> {
    try {
      // Check if config/mats directory exists
      const configHandle = await dirHandle.getDirectoryHandle("config", { create: true });
      const matsHandle = await configHandle.getDirectoryHandle("mats", { create: true });
  ...
  async loadMatConfig(dirHandle: FileSystemDirectoryHandle, matId: string): Promise<GameMatConfig | null> {
    try {
  ...
  async saveMatConfig(dirHandle: FileSystemDirectoryHandle, matId: string, config: GameMatConfig): Promise<void> {
    try {
      // Ensure directory structure exists
      const configHandle = await dirHandle.getDirectoryHandle("config", { create: true });
      const matsHandle = await configHandle.getDirectoryHandle("mats", { create: true });
      const matDirHandle = await matsHandle.getDirectoryHandle(matId, { create: true });
      ...
      // Save mat.json
      const matFileHandle = await matDirHandle.getFileHandle("mat.json", { create: true });
  ...
  async createMatConfig(dirHandle: FileSystemDirectoryHandle, matId: string, config: Omit<GameMatConfig, 'createdAt' | 'updatedAt'>): Promise<void> {
  ...
  async deleteMatConfig(dirHandle: FileSystemDirectoryHandle, matId: string): Promise<void> {
    try {
      const configHandle = await dirHandle.getDirectoryHandle("config");
      const matsHandle = await configHandle.getDirectoryHandle("mats");
  ...
  async matExists(dirHandle: FileSystemDirectoryHandle, matId: string): Promise<boolean> {
    try {
      const configHandle = await dirHandle.getDirectoryHandle("config");
      const matsHandle = await configHandle.getDirectoryHandle("mats");
      const matDirHandle = await matsHandle.getDirectoryHandle(matId);
