export interface FileSystemService {
  ...
  listPythonFiles(dirHandle: FileSystemDirectoryHandle): Promise<FileSystemFileHandle[]>;
  createFile(dirHandle: FileSystemDirectoryHandle, name: string, content: string): Promise<FileSystemFileHandle>;
...
export class WebFileSystemService implements FileSystemService {
  private indexedDBService = import('./indexedDBFileSystem').then(m => m.indexedDBFileSystemService);
  ...
  async listPythonFiles(dirHandle: FileSystemDirectoryHandle): Promise<FileSystemFileHandle[]> {
  ...
  private async searchPythonFiles(dirHandle: FileSystemDirectoryHandle, pythonFiles: FileSystemFileHandle[], depth = 0): Promise<void> {
    ...
    for await (const [name, handle] of dirHandle.entries()) {
      if (handle.kind === 'file' && name.endsWith('.py')) {
        ...
        await this.searchPythonFiles(handle as FileSystemDirectoryHandle, pythonFiles, depth + 1);
      }
    }
  }
  ...
  async watchDirectory(dirHandle: FileSystemDirectoryHandle, callback: (files: FileSystemFileHandle[]) => void): Promise<() => void> {
    let isWatching = true;
  ...
  async createFile(dirHandle: FileSystemDirectoryHandle, name: string, content: string): Promise<FileSystemFileHandle> {
    const fileHandle = await dirHandle.getFileHandle(name, { create: true });
    await this.writeFile(fileHandle, content);
    return fileHandle;
  }

  async persistDirectoryAccess(dirHandle: FileSystemDirectoryHandle): Promise<void> {
    try {
      const indexedDB = await this.indexedDBService;
      await indexedDB.initialize();
