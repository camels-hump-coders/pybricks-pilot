...
function convertSplinePathToCommands(path: SplinePath): SplinePathCommand[] {
  ...
  for (let i = 0; i < path.points.length - 1; i++) {
    ...
    const requiredHeading = normalizeAngle(
      (Math.atan2(dy, dx) * 180) / Math.PI + 90,
    );

    // Calculate turn angle needed
    const turnAngle = calculateTurnAngle(
      currentPoint.position.heading,
      requiredHeading,
    );

    // Add turn command if needed
    if (Math.abs(turnAngle) > 5) {
      // Only turn if angle is significant
      commands.push({
        type: "turn",
        angle: turnAngle,
    ...
    // Add drive command
    if (distance > 5) {
      // Only drive if distance is significant
      commands.push({
        type: "drive",
        distance: distance,
 ...
 */
function convertSplinePathToSmoothCommands(
  path: SplinePath,
): SplinePathCommand[] {
  if (!path || path.points.length < 2) {
    return [];
  ...
  for (let i = 0; i < path.points.length - 1; i++) {
    ...
    if (currentPoint.tangencyHandle || nextPoint.tangencyHandle) {
      ...
      if (nextPoint.tangencyHandle) {
        ...
        if (currentPoint.tangencyHandle) {
          arcRadius =
            (arcRadius + Math.max(30, handleLength * (1 - strength * 0.5))) / 2;
    ...
    // Priority 2: Use manual control points if available
      ...
      currentPoint.controlPoints?.after ||
      nextPoint.controlPoints?.before
    ) {
      ...
      const startAngle = normalizeAngle(
        (Math.atan2(cp1.y, cp1.x) * 180) / Math.PI,
      );
      const endAngle = normalizeAngle(
        (Math.atan2(-cp2.y, -cp2.x) * 180) / Math.PI,
      );
...
export async function executeSplinePath(
  ...
  const robotCommands = pathCommands
    .flatMap((cmd) => {
      switch (cmd.type) {
        case "turn":
          return {
