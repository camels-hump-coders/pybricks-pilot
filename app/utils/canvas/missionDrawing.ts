...
export function drawMissions(
  ...
  customMatConfig.missions.forEach((mission) => {
    ...
    if (completionPercentage > 0) {
      ctx.fillStyle = isHovered ? "rgba(0, 255, 0, 1)" : "rgba(0, 255, 0, 0.9)";
      ctx.beginPath();
...
function drawHeadingIndicator(
  ...
  const endX =
    pos.x + Math.sin(headingRadians) * (arrowStartDistance + arrowLength);
  const endY =
    pos.y - Math.cos(headingRadians) * (arrowStartDistance + arrowLength);
...
function drawMissionArcPaths(
  ...
  segments.forEach((segment, _index) => {
    ...
    if (showArrows && canvasPoints.length >= 2) {
      ...
      const nextPoint =
        canvasPoints[Math.min(midIndex + 1, canvasPoints.length - 1)];
...
export function drawMissionPathPreview(
  ...
  const previewPoint: MissionPointType = {
    ...
    type: pointType,
    ...(pointType === "action"
      ? { heading: actionHeading, actionName: "Preview", pauseDuration: 1 }
      : {}),
  } as MissionPointType;
  ...
  drawMissionPathPreviewSegments(
    ctx,
    mission,
    previewMission,
    insertIndex,
