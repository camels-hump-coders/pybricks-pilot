...
export function drawSplinePath(
  ...
  if (splinePath.points.length > 1) {
    ctx.beginPath();
    const firstPoint = mmToCanvas(
      splinePath.points[0].position.x,
      splinePath.points[0].position.y,
    );
    ...
    for (let i = 1; i < splinePath.points.length; i++) {
      ...
      const canvasCurrentPoint = mmToCanvas(
        currentPoint.position.x,
        currentPoint.position.y,
      );
      const canvasPrevPoint = mmToCanvas(
      ...
      if (prevPoint.tangencyHandle || currentPoint.tangencyHandle) {
        ...
        ctx.bezierCurveTo(
          ...
          cp2.x,
          cp2.y,
          canvasCurrentPoint.x,
          canvasCurrentPoint.y,
        );
      }
      // Priority 2: Use manual control points if available
      else if (
        prevPoint.controlPoints?.after ||
        currentPoint.controlPoints?.before
      ) {
  ...
  splinePath.points.forEach((point, index) => {
    ...
    if (point.tangencyHandle) {
      ...
      const isHoveredTangencyHandle =
        hoveredCurvatureHandlePointId === point.id;
  ...
  for (const point of splinePath.points) {
    ...
    if (point.controlPoints.before) {
      ...
      const distance = Math.sqrt(
        (mouseCanvasX - controlPos.x) ** 2 + (mouseCanvasY - controlPos.y) ** 2,
      );
...
export function findClickedTangencyHandle(
  ...
  for (const point of splinePath.points) {
    ...
    const diamondDistance = Math.sqrt(
      (mouseCanvasX - diamondPos.x) ** 2 + (mouseCanvasY - diamondPos.y) ** 2,
    );
    ...
    const arrowDistance = Math.sqrt(
      (mouseCanvasX - arrowPos.x) ** 2 + (mouseCanvasY - arrowPos.y) ** 2,
    );
    ...
    const endDistance = Math.sqrt(
      (mouseCanvasX - endPos.x) ** 2 + (mouseCanvasY - endPos.y) ** 2,
    );
