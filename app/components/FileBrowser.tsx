      ...
      <div className="max-h-96 overflow-y-auto">
        {pythonFiles.length === 0 ? (
          <div className="p-8 text-center text-gray-500">
            ...
            <p className="text-sm">
              {isLoading ? 'Scanning directory...' : 'Create a new file or add .py files to this directory'}
            </p>
          </div>
        ) : (
          <div>
      ...
      <div className="p-3 bg-gray-50 border-t text-xs text-gray-600">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <span>
              {pythonFiles.length} Python file{pythonFiles.length !== 1 ? 's' : ''} found
            </span>
            <span className="text-purple-600">● Persisted to IndexedDB</span>
          </div>
          <span>
            {isLoading && <span className="text-blue-600">● Scanning...</span>}
            {isRestoring && <span className="text-orange-600">● Restoring from storage...</span>}
            {selectedFile && <span className="text-green-600">● {selectedFile.name} selected</span>}
          </span>
        </div>
      </div>
    </div>
