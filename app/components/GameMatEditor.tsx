      ...
      reader.onload = (e) => {
        try {
          ...
          if (config.version === "1.0") {
            ...
            setOriginalImageData(config.originalImageData || config.imageData);
            if (config.corners) {
...
${missions.map((obj, i) => `${i + 1}. **${obj.name}** - ${obj.objectives.length} objective${obj.objectives.length !== 1 ? "s" : ""} (${obj.objectives.reduce((sum, o) => sum + o.points, 0)} points)`).join("\\n")}
              ...
              <div>
                ...
                <div className="space-y-2">
                  {scoringObjects.map((obj) => (
                    ...
                    >
                      {obj.name} (
                      {obj.objectives.reduce((sum, o) => sum + o.points, 0)}pts)
                    </button>
                  ))}
                </div>
              </div>
                    ...
                    {missions.reduce(
                      (sum, obj) =>
                        ...
                        obj.objectives.reduce(
                          (objSum, objective) => objSum + objective.points,
                          0
                        ),
                      0
                    )}
