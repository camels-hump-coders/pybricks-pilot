...
export function SensorDisplay({ sensorData, className = '' }: SensorDisplayProps) {
  if (!sensorData || Object.keys(sensorData).length === 0) {
    return (
      <div className={`space-y-2 ${className}`}>
        <h3 className="text-lg font-semibold text-gray-700 dark:text-gray-300">Sensors</h3>
        <p className="text-sm text-gray-500 dark:text-gray-400">No sensor data available</p>
      </div>
    );
  }
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200">Sensors</h3>
        ...
        {Object.entries(sensorData).map(([name, data]) => (
          ...
          >
            ...
            <div className="text-center">
              <div className={`text-lg font-mono font-bold ${data.error ? 'text-red-800 dark:text-red-300' : 'text-gray-800 dark:text-gray-200'}`}>
              ...
              {!data.error && data.type.toLowerCase() === 'color' && (
                <div className="mt-2 text-xs text-gray-600 dark:text-gray-400 space-y-1">
                  {data.reflection !== undefined && <div>Reflection: {data.reflection}%</div>}
                  {data.ambient !== undefined && <div>Ambient: {data.ambient}%</div>}
                </div>
              )}
            </div>

            {/* Special visualizations for certain sensor types */}
            {data.type.toLowerCase() === 'color' && typeof data.value === 'object' && Array.isArray(data.value) && (
            ...
            {data.type.toLowerCase() === 'ultrasonic' && data.distance !== undefined && (
              <div className="mt-2">
