...
export function PositionManagementDialog() {
  ...
  const [editingPosition, setEditingPosition] = useState<NamedPosition | null>(null);
  ...
  const handleEditSave = async () => {
    ...
    const nameExists = positions.some(
      pos => pos.id !== editingPosition.id && pos.name.toLowerCase() === trimmedName.toLowerCase()
    );
    ...
    const success = await updateCustomPosition(editingPosition.id, { name: trimmedName });
    if (success) {
      setEditingPosition(null);
      setEditName("");
      setError("");
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
              Position management requires a mounted folder to save configuration files. 
              Please mount a folder to enable position management.
            </p>
            <button
              onClick={handleClose}
                ...
                >
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                      X: {Math.round(position.x)}mm, Y: {Math.round(position.y)}mm, θ: {Math.round(position.heading)}°
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
