...
export function DrivebaseDisplay({
  drivebaseData,
  className = "",
  isCompact = false,
}: DrivebaseDisplayProps) {
    ...
    return (
      <div className={`space-y-2 ${className}`}>
        <div className="grid grid-cols-2 gap-2 text-xs">
          ...
          <div className="text-center">
            <div className="text-gray-500 dark:text-gray-400">Angle</div>
            <div className="font-mono font-bold text-gray-800 dark:text-gray-200">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs pt-2 border-t border-gray-200 dark:border-gray-700">
            <div className="text-center">
              ...
              <div className="font-mono text-sm text-gray-800 dark:text-gray-200">
                {formatValue(drivebaseData.state.drive_speed, "", 0)}
              </div>
            </div>
            <div className="text-center">
              <div className="text-gray-500 dark:text-gray-400">Turn</div>
              <div className="font-mono text-sm text-gray-800 dark:text-gray-200">
                {formatValue(drivebaseData.state.turn_rate, "", 0)}
              </div>
            </div>
          </div>
        )}
            ...
            <div className="flex items-center justify-center gap-2">
              <div
                className={`w-2 h-2 rounded-full ${
                  Math.abs(drivebaseData.state.drive_speed) > 5 ||
                  Math.abs(drivebaseData.state.turn_rate) > 5
                    ? "bg-green-500"
