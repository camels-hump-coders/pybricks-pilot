  ...
  (get, set, debugEvents: DebugEvent[]) => {
    ...
    for (const event of recentEvents.reverse()) {
      if (event.type === 'upload') {
        ...
        const progressMatch = event.message.match(/Progress: chunk (\d+)\/(\d+)/);
        if (progressMatch) {
        ...
        if (event.message.includes('Upload completed') || 
        ...
        if (event.message.includes('Starting') && (event.message.includes('upload') || event.message.includes('compilation'))) {
 ...
 */
export const resetUploadProgressAtom = atom(
  null,
  (get, set) => {
