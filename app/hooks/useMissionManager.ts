...
import type { Mission, MissionsFileData, MissionPointType } from "../types/missionPlanner";
...
export function useMissionManager() {
  ...
  const [selectedMissionId, setSelectedMissionId] = useAtom(selectedMissionIdAtom);
  ...
  // UI state
  const [isMissionManagementOpen, setIsMissionManagementOpen] = useAtom(isMissionManagementOpenAtom);
  const [isAddMissionDialogOpen, setIsAddMissionDialogOpen] = useAtom(isAddMissionDialogOpenAtom);
  const [isMissionEditorOpen, setIsMissionEditorOpen] = useAtom(isMissionEditorOpenAtom);
  ...
  const handleRemoveMission = useCallback(async (missionId: string): Promise<boolean> => {
    try {
  ...
  const handleStartEditingMission = useCallback((missionId: string) => {
  ...
  const handleUpdatePoint = useCallback((pointId: string, updates: Partial<MissionPointType>) => {
  ...
  // Get mission by ID
  const getMissionById = useCallback((missionId: string): Mission | null => {
    return missions.find(mission => mission.id === missionId) || null;
  }, [missions]);

  // Validation functions
  const validateMissionName = useCallback((name: string, excludeMissionId?: string): string | null => {
    if (!name.trim()) {
    ...
    const existingMission = missions.find(m => 
      m.id !== excludeMissionId && m.name.toLowerCase() === name.trim().toLowerCase()
    );
