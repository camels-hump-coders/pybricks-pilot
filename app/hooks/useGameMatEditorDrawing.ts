...
export function useGameMatEditorDrawing(props: UseGameMatEditorDrawingProps) {
  ...
  const lerp2D = useCallback((p1: Point, p2: Point, t: number): Point => {
    return {
      x: p1.x + (p2.x - p1.x) * t,
      y: p1.y + (p2.y - p1.y) * t,
    };
  ...
  const drawPerspectiveGrid = useCallback(
    (
      ctx: CanvasRenderingContext2D,
      imgX: number,
      imgY: number,
  ...
  const drawCalibrationPoint = useCallback(
    (
      ctx: CanvasRenderingContext2D,
      x: number,
      y: number,
  ...
  const drawCalibrationPoints = useCallback(
    (
      ctx: CanvasRenderingContext2D,
      imgX: number,
      imgY: number,
  ...
  const drawMissions = useCallback(
    (
      ctx: CanvasRenderingContext2D,
      imgX: number,
      imgY: number,
  ...
  const drawCanvas = useCallback(() => {
    ...
    if (
      (data.mode === "objects" || data.mode === "preview") &&
