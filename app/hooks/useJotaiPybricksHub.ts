...
import type { TelemetryData, ProgramStatus } from "../services/pybricksHub";
...
export function useJotaiPybricksHub() {
  ...
  }, [setTelemetryData, setProgramStatus, setProgramOutputLog, setHubInfo, setIsConnected]);
  ...
  const sendDriveCommand = useCallback(async (distance: number, speed: number) => {
  ...
  const sendContinuousDriveCommand = useCallback(async (speed: number, turnRate: number) => {
    const command = JSON.stringify({
  ...
  const sendMotorCommand = useCallback(async (motor: string, angle: number, speed: number) => {
    const command = JSON.stringify({
      action: "motor",
      motor,
      angle,
  ...
  const sendContinuousMotorCommand = useCallback(async (motor: string, speed: number) => {
    const command = JSON.stringify({
      action: "motor",
      motor,
      speed,
